...
def test_core_features_imputes_missing_gap_and_flags_it():
    with_second_line = core_features(300, True, 150, 20, True, 3)
    assert with_second_line == [300.0, 1.0, 1.0, 150.0, 20.0, 1.0, 3.0]

    # No second line at all — gap imputed to 0, flag set to 0, not 1.
    without_second_line = core_features(50, False, None, -400, False, 0)
    assert without_second_line == [50.0, 0.0, 0.0, 0.0, -400.0, 0.0, 0.0]


def test_build_core_feature_matrix_stacks_examples_in_order():
    examples = [
        FakeExample(300, True, 150, 20, True, 3),
        FakeExample(50, False, None, -400, False, 0),
    ]
    ...
    assert list(X[1]) == [50.0, 0.0, 0.0, 0.0, -400.0, 0.0, 0.0]
